...
async def testar_e_mostrar_json():
    print("🔄 Executando teste do Itaú...")
    from teste_normalizacao_etl import testar_normalizacao_banco

    # Executar teste e capturar resultado
    resultado = await testar_normalizacao_banco('Itaú', r'c:\Users\gabri\Downloads\Extratos\Itau_Pj.pdf')
    ...
    if resultado and 'dados' in resultado:
        ...
        if dados.get('transacoes_finais'):
            ...
            with open('exemplo_extrato_itau.json', 'w', encoding='utf-8') as f:
                json.dump(dados, f, indent=2, ensure_ascii=False, default=str)
            print('\n💾 Dados normalizados salvos em: exemplo_extrato_itau.json')
            ...
            # Gerar JSON ETL
            json_etl_itau = gerar_json_etl_extratos(dados, transacoes_brutas, metadados_extracao)

            # Salvar JSON ETL
            with open('exemplo_etl_itau.json', 'w', encoding='utf-8') as f:
                f.write(json_etl_itau)
            print('💾 JSON ETL de entrada salvo em: exemplo_etl_itau.json')
    else:
